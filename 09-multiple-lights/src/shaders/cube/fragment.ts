const fragmentSrc = `
    ...
    vec3 calcDirLight(DirLight light, Material material, vec3 normal, vec3 viewDir, vec2 texCord) {
        ...
        vec3 diffuse = light.diffuse * texture(material.diffuse, texCord).rgb * diff;
        vec3 specular = light.specular * texture(material.specular, texCord).rgb * spec;
    ...
    vec3 calcPointLight(PointLight light, Material material, vec3 normal, vec3 viewDir, vec2 texCord, vec3 fragPos) {
        ...
        vec3 ambient = light.ambient * texture(material.diffuse, texCord).rgb * attenuation;
        vec3 diffuse = light.diffuse * texture(material.diffuse, texCord).rgb * diff * attenuation;
        vec3 specular = light.specular * texture(material.specular, texCord).rgb * spec * attenuation;
    ...
    vec3 calcSpotLight(SpotLight light, Material material, vec3 normal, vec3 viewDir, vec2 texCord, vec3 fragPos) {
        ...
        vec3 ambient = light.ambient * vec3(texture(material.diffuse, vTexCord)) * attenuation;
        vec3 diffuse = light.diffuse * vec3(texture(material.diffuse, vTexCord)) * diff * attenuation * intensity;
        vec3 specular = light.specular * vec3(texture(material.specular, vTexCord)) * spec * attenuation * intensity;
        return ambient + diffuse + specular;
    }
`.trim();
