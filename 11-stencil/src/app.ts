...
import { lampVertexSrc, lampFragSrc, cubeVertexSrc, cubeFragSrc, borderCubeVertexSrc, borderCubeFragSrc } from './shaders/index';
...
Promise.all([texLoader('../images/container.png'), texLoader('../images/container_specular.png')])
    .then(([img1, img2]) => {
        ...
        for (let i = 0; i < 9; i++) {
            ...
            const cubeModel = new Model(gl, cubeMesh, cubeModelMx, true);
...
new RenderLooper(msdt => {
    ...
    borderCubeShader.use();
    borderCubeShader.uniformMatrix4fv('uView', viewMx);
    borderCubeShader.uniformMatrix4fv('uPerspective', perspeciveMx);
